...
- **Wire `Sitemap` + add `/robots.txt`.** `project_name/pages/sitemaps.py-tpl` defines the sitemap but it is not registered in `project_name/urls.py-tpl`, and there is no `robots.txt` view or static file anywhere in the tree. For an SEO-led customer-acquisition strategy (per `MOTIVATION.md`), this is a free, high-leverage win that every generated product is currently missing on day one. Add a sitemap URL under the public urls, a `robots.txt` view that respects `DEBUG`/staging hosts (deny all on non-prod), and a test that asserts both respond 200 in prod-like settings.
...
- **Add automated dependency updates and a secrets scan to CI.** There is no `.github/dependabot.yml` / `renovate.json`, and `.github/workflows/ci.yml` runs no secret-leak step. For a portfolio of long-running SaaS sites with a single maintainer, both are foundational - missing them is the kind of thing a TDD reviewer flags immediately. Add Dependabot (weekly, grouped, pip + npm + github-actions + docker) and a `gitleaks` step in CI. Optionally add `pip-audit`/`uv tool run pip-audit` as a non-blocking job.

## P1 — important for EU B2B readiness

- **Cookie/consent layer for non-essential trackers.** Plausible is cookie-free, so this template is closer to compliant than most, but the `core/templates/` layout already injects scripts unconditionally and there is no consent surface. The minimum acceptable state for EU B2B is: (a) a documented "we only use first-party cookies" stance on a `cookies` page, (b) a thin `<c-consent>` component that gates _any_ third-party script (e.g. Chatwoot, future ad pixels) behind a per-team / per-visitor consent flag, and (c) a `consent_required` template tag that wraps third-party `<script>` tags. Keep it minimal - this is plumbing, not a feature.
...
- **Backup, restore, and a _tested_ restore drill.** `DEPLOYMENT.md` honestly says backups are not scaffolded. For a portfolio of unattended SaaS products, the cheapest possible "backups + restore drill" is: a `just db-backup` recipe that pipes `pg_dump` to age-encrypted output on object storage, plus a `just db-restore-drill` that pulls the latest backup into a throwaway Postgres and runs `manage.py migrate --check`. Add both, plus a `docs/backups.md` covering retention, encryption, and an actual restore-drill cadence. Data loss is the only failure mode `MOTIVATION.md` explicitly calls out as unacceptable.
